...
[类](/5-classes/1-class/) 可以让其他的类复用它的属性和方法，这在 TypeScript 中被称为继承。继承其他类的属性和方法的类被称为子类，被继承的类被称为父类，这些名字来自自然中孩子继承父母基因的说法。继承让你可以复用现有类的功能，而不需要重写一遍。

JavaScript 使用 [原型继承](/5-classes/1-class/) 的方式实现类，而非 `Java` 和 `C#` 语言的类继承方式。 `ES6` 引入的 [类](/5-classes/1-class/) 语法是 JavaScript 原型继承的语法糖，TypeScript 也支持这种语法。
